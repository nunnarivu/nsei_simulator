...
class Entity(object):
    def __init__(self, physics_client, name:str, pos:tuple, ori:tuple, model_path:str = None, **kwargs):
    ...
    def reset_pose(self, pos = None, ori = None):
        if pos is not None:
            self.pos = pos
        if ori is not None:
            self.ori = ori
        self.physics_client.resetBasePositionAndOrientation(self.id, self.pos, self.ori)
...
class Object(Entity):
    def __init__(self, physics_client, name:str, pos:tuple, ori:tuple, model_path:str, obj_id:int, **kwargs):
        super().__init__(physics_client, name, pos, ori, model_path, **kwargs)
        self.obj_id = obj_id
...
class SpecialObject(Object):
    def __init__(self,physics_client, name:str, pos:tuple, ori:tuple, model_path:str, obj_id:int, tag:str, **kwargs):
        super().__init__(physics_client, name, pos, ori, model_path, obj_id, **kwargs)
        self.tag = tag
        self.attributes = {}
...
class WorldUtils(object):
    ...
    def _add_object(self, name:str, pos:tuple, ori:tuple, model_path:str, **kwargs):
        '''
        ...
        obj = Object(self.physics_client, name, pos, ori, model_path, obj_id, **kwargs)
        self.object_list.append(obj)
        return obj

    def _add_entity(self, name:str, pos:tuple, ori:tuple, model_path:str = None, **kwargs):
        '''
        ...
        entity = Entity(self.physics_client, name, pos, ori, model_path, **kwargs)
        self.entity_list.append(entity)
        return entity

    def _add_special_object(self, name:str, pos:tuple, ori:tuple, model_path:str, tag:str, **kwargs):
        ...
        obj = SpecialObject(self.physics_client, name, pos, ori, model_path, obj_id, tag, **kwargs)
        self.special_object_list.append(obj)
        return obj

    def _add_robot(self, name:str, pos:tuple, ori:tuple, model_path:str, robot_cls, **kwargs):
        robot = robot_cls(self.physics_client, name, pos, ori, model_path, **kwargs)
...
class World(WorldUtils):
    ...
    def add(self, name:str, pos:tuple, ori:tuple, obj_type:str = 'object', **kwargs):
        ...
        if obj_type == 'object':
            return self._add_object(name, pos, ori, model_path, **kwargs['other_properties'])
            ...
            return self._add_special_object(name, pos, ori, model_path, kwargs.get('tag'), **kwargs['other_properties'])
        elif obj_type == 'robot':
            if kwargs.get('robot_cls') is None:
                raise ValueError("Robot class must be provided for robots")
            return self._add_robot(name, pos, ori, model_path, kwargs.get('robot_cls'), **kwargs['other_properties'])
        else:
