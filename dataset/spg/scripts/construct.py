...
import dataset.spg.configs as configs
from dataset.spg.configs import ParameterSettings as settings
from dataset.spg.construct.program_generator import ProgramGenerator
...
def construct_main(template_file:str, metadata_file:str, dataset_dir:str):
    ...
    Generator = DataConstructor(configs.simulator_configs, configs.required_dataset_configs)
...
class DataConstructor(object):
    def __init__(self, simulator_configs:dict, required_dataset_configs:dict):
        
        self.construct_handle = ProgramGenerator(simulator_handle = init_simulator_handle('bullet', simulator_configs), 
                                                 configs = simulator_configs | required_dataset_configs)
    ...
    def construct_data(self, dataset_dir:str, template_file:str, metadata_file:str):
        ...
        self.construct_handle.initialize_template()
        self.construct_handle.generate_initial_scene()
        ...
        status = self.construct_handle.generate_grounded_functional_program()
        if status == False:
            os.system(f"rm -rf {smpl_dir}")
            print(f"Failed to generate a program for sample {sample_no}. Skipping this sample.")
            
            
        command_lexed, command = self.construct_handle.generate_instruction()
        self.construct_handle.save_demonstration_info(command_lexed, command)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_distractors', type=int, default=None, help='Number of distractor objects to add to the scene')
